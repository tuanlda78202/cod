...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), torch.stack(dec_out_ori_logits)
...
class RTDETRTransformer(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        # print("enc_outputs_class",enc_outputs_class.shape)

        _, topk_ind = torch.topk(enc_outputs_class.max(-1).values, self.num_queries, dim=1)

        # print("topk_ind",topk_ind.shape)
        ...
        ori_enc_topk_logits = output_memory.gather(dim=1, \
                                                   index=topk_ind.unsqueeze(-1).repeat(1, 1,
                                                                                       enc_outputs_class.shape[-1]))
        ...
        # print("enc_topk_logits",enc_topk_logits.shape)

        return target, reference_points_unact.detach(), enc_topk_bboxes, enc_topk_logits, ori_enc_topk_logits
    ...
    def forward(self, feats, targets=None):
        ...
        target, init_ref_points_unact, enc_topk_bboxes, enc_topk_logits, ori_enc_topk_logits = \
            self._get_decoder_input(memory, spatial_shapes, denoising_class, denoising_bbox_unact)

        # print("target shape",target.shape)

        # abc
        out_bboxes, out_logits, out_ori_logits = self.decoder(
            target,
        ...
        # print("out_logits.shape",out_logits[-1].shape)
        ...
        # for i in range (len(out['dn_aux_outputs'])):
        # print("out['dn_aux_outputs'] number ",i,out['dn_aux_outputs'][i]['pred_logits'].shape)
        # print(out['origin_logits'].shape)
        return out
