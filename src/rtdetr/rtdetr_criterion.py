...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses, alpha=0.2, gamma=2.0, eos_coef=1e-4, num_classes=80):
        ...
        self.alpha = alpha
        self.gamma = gamma
        self.proxy = ProxyNCA(80, 256).cuda()
    ...
    def loss_proxy(self, outputs, targets, indices, num_boxes, log=True):
        ...
        assert 'origin_logits' in outputs
        src_logits = outputs['origin_logits'].float()
        a=0

        if src_logits.shape!=torch.Size([8, 300, 80]):
            ...
            for i in range(src_logits.shape[0]):
                a+=self.proxy(src_logits[i],target[i])/ num_boxes*3
