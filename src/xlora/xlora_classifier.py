...
class xLoRAClassifier(nn.Module):
    ...
    def __init__(
        ...
        if self.config.xlora_depth == 1:
            if config.layerwise_scalings:  # bias=False if we have just one layer
                self.last = (
                    nn.Linear(config.hidden_size, n_classes * n_layers, bias=bias_flag)
                    .to(config.device)
                    .to(dtype)
                )
            else:
                self.last = (
                    nn.Linear(config.hidden_size, n_classes, bias=bias_flag)
                    .to(config.device)
                    .to(dtype)
                )
        elif self.config.xlora_depth == 2:
            self.inner.append(
                nn.Linear(config.hidden_size, config.xlora_size, bias=bias_flag)
                .to(config.device)
                .to(dtype)
            )
            ...
            if config.layerwise_scalings:
                self.last = (
                    nn.Linear(config.xlora_size, n_classes * n_layers, bias=bias_flag)
                    .to(config.device)
                    .to(dtype)
                )
            else:
                self.last = (
                    nn.Linear(config.xlora_size, n_classes, bias=bias_flag)
                    .to(config.device)
                    .to(dtype)
                )
        else:
            ...
            for _ in range(config.xlora_depth - 2):
                self.inner.append(
                    nn.Linear(config.xlora_size, config.xlora_size, bias=bias_flag)
                    .to(config.device)
                    .to(dtype)
                )
    ...
    def flush_log_scalings(self, path: str):
        ...
        if len(seqlens_map) == 1:
            self._save_scalings(
                path, [scaling.unsqueeze(0) for scaling in self.log_scalings]
            )
        else:
            ...
            for seq_len, (indices, scalings_list) in seqlens_map.items():
                ...
                self._save_scalings(
                    f"{path}-{seq_len}",
                    [scaling.unsqueeze(0) for scaling in scalings_list],
                )
