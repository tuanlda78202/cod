...
def load_model(
    ...
    Union[AutoModelForCausalLM, xLoRAModel],
    ...
    if not os.path.exists(model_name):
        ...
        xlora_classifier = [
            ...
            if "xlora_classifier.safetensors" in name or "xlora_classifier.pt" in name
        ][0]
        ...
        if "adapter_config" in adapter_names:
            raise ValueError(
                "Got adapter_config in the adapter names. That should not be there."
            )
        adapter_paths = {}
        ...
        if adapters is None:
            ...
            for adapter_name in adapter_names:
                ...
                huggingface_hub.hf_hub_download(
                    model_name, adapter_filename, subfolder=adapter_name
                )
                cfg_filename = [
                    name
                    for name in [file["name"][len(adapter_name_path) + 1 :] for file in s.ls(adapter_name_path)]  # type: ignore
                ...
                huggingface_hub.hf_hub_download(
                    model_name, cfg_filename, subfolder=adapter_name
                )
    ...
    with open(
        (
            ...
            else os.path.join(model_name, "xlora_config.json")
        ),
        "r",
    ) as f:
        conf = json.load(f)
