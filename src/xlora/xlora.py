...
def convert_layers_to_xlora(
    ...
    for module in base.modules():
        if isinstance(module, lora.Linear):
            new_layer: Union[
                xLoRALinearLayer, xLoRAEmbeddingLayer, xLoRAConv2dLayer
            ] = xLoRALinearLayer(
                model=base,
                target=module,
    ...
    **kwargs,
    ...
    if verbose:
        if "subfolders" in kwargs:
            adapters_items = iter(
                tqdm.tqdm(zip(xlora_config.adapters.items(), kwargs["subfolders"]))
            )
        else:
        ...
        if "subfolders" in kwargs:
            adapters_items = iter(
                zip(xlora_config.adapters.items(), kwargs["subfolders"])
            )
        else:
    ...
    if "subfolders" in kwargs:
        model_peft = PeftModel.from_pretrained(
            ...
            first_item[0][1],
            first_item[0][0],
            is_trainable=use_trainable_adapters,
            subfolder=first_item[1],
        )
    else:
        model_peft = PeftModel.from_pretrained(
            model,
    ...
    def hook(module, *args, **kwargs) -> None:
        ...
        if "_xlora_classifier_inhibitor_flag" in kwargs_real:
            payload: InhibitorFlagPayload = kwargs_real[
                "_xlora_classifier_inhibitor_flag"
            ]
    ...
    xlora_classifier = xLoRAClassifier(
        model_peft, xlora_config, n_classes, total_swapped
    )
    ...
    **kwargs,
    ...
    if not os.path.exists(load_directory):
        config_path = huggingface_hub.hf_hub_download(
            repo_id=load_directory, filename="xlora_config.json"
        )
        if from_safetensors:
            ...
            classifier_path = huggingface_hub.hf_hub_download(
                repo_id=load_directory, filename="xlora_classifier.pt"
            )
    ...
    with open(
        (
            ...
            else os.path.join(load_directory, "xlora_config.json")
        ),
        "r",
    ) as f:
        conf = json.load(f)
    ...
    if from_safetensors:
        ...
        state_dict = torch.load(
            classifier_path
            if classifier_path is not None
            else os.path.join(load_directory, "xlora_classifier.pt")
        )
